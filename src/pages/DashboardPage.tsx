    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center gap-3 shrink-0 shadow-sm">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          {editingName ? (
            <div className="flex items-center gap-2">
              <input
                ...
                className="text-base font-bold text-slate-900 border-b-2 border-brand-500 outline-none bg-transparent px-1 min-w-56"
              />
              <button onClick={commitName} className="p-1 text-emerald-600 hover:bg-emerald-50 rounded-md">
                <Check className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => setEditingName(false)} className="p-1 text-slate-400 hover:bg-slate-100 rounded-md">
                <X className="w-3.5 h-3.5" />
              </button>
            </div>
          ) : (
            <div className="flex items-center gap-1.5 group min-w-0">
              <h2 className="text-base font-bold text-slate-900 truncate tracking-tight">{dashboard.name}</h2>
              <button
                onClick={() => setEditingName(true)}
                className="p-1 text-slate-300 hover:text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity rounded hover:bg-slate-100 shrink-0"
              >
          ...
          {dirty && !saved && (
            <span className="text-[11px] text-amber-600 font-semibold bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full shrink-0">
              Unsaved
            </span>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto bg-slate-100">
        <div className="max-w-[1320px] mx-auto p-6 flex flex-col gap-5">
          {/* AI Prompt */}
          <AIPromptBar
            dashboard={dashboard}
