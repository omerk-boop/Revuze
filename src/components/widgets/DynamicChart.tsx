    ...
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={spec.chartData} margin={{ top: 4, right: hasRight ? 16 : 8, left: -8, bottom: 0 }}>
        ...
        {spec.series.map((s, i) => {
          ...
          if (s.kind === 'area') return <Area {...commonProps} stroke={color} fill={color} fillOpacity={0.15} strokeWidth={2} dot={false} />
          return <Line {...commonProps} stroke={color} strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
        })}
      </ComposedChart>
    </ResponsiveContainer>
  )
